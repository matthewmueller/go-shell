...
`go-shell` is a small Go package that wraps `os/exec`, allowing you to centrally
manage defaults like the environment and stdio.
...
func main() {
  ...
  ctx := context.Background()
	if err := exec.Command("go", "version").Run(ctx); err != nil {
		log.Fatal(err)
	}
}
```
...
- `New() *Exec`: creates an executor with inherited stdio and environment.
